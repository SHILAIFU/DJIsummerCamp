...
Mat boxMode  = imread("/home/lu/catkin_ws2/src/Guidance-SDK-ROS/led_picture/1.png");
Mat boxMode2 = imread("/home/lu/catkin_ws2/src/Guidance-SDK-ROS/led_picture/2.png");
...
void boxDetect(Mat img, vector<Point> boxModeContour)
{
    Mat imgGray;
    cvtColor(img, imgGray, CV_BGR2GRAY);
    ...
    for(int i=0;i<contours.size();i++)
    {
        ...
        if(contArea > 3000 && isContourConvex(Mat(contourDP)) && contourDP.size() == 4)
        {
            ...
            if(similarityParam<0.01)
            {
                ...
                solvePNP(vector<Point> pointsIn);
            }
        }
    }
    imshow("src", img);
...
void solvePNP(vector<Point> pointsIn)
{
    vector<Point> objectPoints;//Size of the Marker
    ...
    __aruco_solve_pnp(objectPoints,imagePoints,cameMatrix,distCoeffs,rotat_vecr1,trans_vecr1);
    uav_vision::DetectInfo boxInfo;
    std_msgs::Float32MultiArray boxData;
    boxData.data.push_back(trans_vecr1.at<double>(0,0));
    boxData.data.push_back(trans_vecr1.at<double>(0,1));
    boxData.data.push_back(trans_vecr1.at<double>(0,2));
    boxData.data.push_back(rotat_vecr1.at<double>(0,0));
    boxData.data.push_back(rotat_vecr1.at<double>(0,1));
    boxData.data.push_back(rotat_vecr1.at<double>(0,2));
    cout   << "x= " << r0 << endl
           << "y= " << r1 << endl
           << "z= " << r2 << endl;
...
{
    Mat img = msg -> image;
    boxDetect(img, boxModeContour);
    boxDetect(img, boxModeContour2);
    boxDetect(img, boxModeContour3);
}

int main(int argc, char** argv)
{
    ros::init(argc, argv, "uav_box_detect");
    ros::NodeHandle nh;
    ros::Subscriber subImg;
    cout << "The box detection node is setup!" << endl;
    pubLedPos = nh.advertise<uav_vision::DetectInfo>("/uav_vision/detectBox", 1000);
    pubLedImgeInfo = nh.advertise<cv_bridge::CvImage>("/uav_vision/infoBox", 2);
