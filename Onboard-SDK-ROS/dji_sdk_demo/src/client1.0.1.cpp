        ...
        switch(main_operate_code)
        {
            ...
            case 'g':
                ...
                sleep(8);
                    ...
                    drone->velocity_control(0,0.2,0,0,0);
                    usleep(20000);
                    ROS_INFO("trun left!");
                    sleep(8);

                last_time=ros::Time::now().sec;
//                while((ros::Time::now().sec-last_time)<10.0)
//                {
//                    t=ros::Time::now().sec-last_time;
//                    drone->velocity_control(0,-0.2,0,0,0);
//                    usleep(20000);
//                    ROS_INFO("trun right! time:%d", (int)t);
//                }
                while((ros::Time::now().sec-last_time)<6.0)
                {
                    t=(ros::Time::now().sec-last_time);
                    drone->velocity_control(0,0.2,0,0,0);
                    usleep(20000);
                    ROS_INFO("trun left! time: %d", (int)t);
                }
                 sleep(3);
                last_time=ros::Time::now().sec;
                while((ros::Time::now().sec-last_time)<6.0)
                ...
                {
                    t=ros::Time::now().sec-last_time;
                    drone->velocity_control(0,0,0,0.2,0);
                    usleep(20000);
                    ROS_INFO("up! time: %d", (int)t);
                }
                 sleep(3);
                last_time=ros::Time::now().sec;
                while((ros::Time::now().sec-last_time)<6.0)
